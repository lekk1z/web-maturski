  ...
  <div class="rezervacije-container">
    ...
    <form @submit.prevent="submitReservation" class="reservation-form">
      ...
      <div>
        ...
        <input v-model="form.name" id="name" type="text" required placeholder="Unesite ime gosta" />
      </div>
      <div>
      ...
      <button type="submit" :disabled="submitting || loadingTables">Rezerviši</button>
      <span v-if="submitSuccess" style="color:green; margin-left:10px;">Rezervacija uspešno poslata!</span>
      <span v-if="submitError" style="color:red; margin-left:10px;">{{ submitError }}</span>
    </form>
    <div v-if="loadingTables" style="margin-top:12px;">Učitavanje stolova...</div>
